...
* Excess kurtosis = -0.95, meaning the distribution is platykurtic — flatter than a normal distribution (0).
...
* Investigating the outlier peak bin [ca. 27,000 nM], there are 20585 rows all with Standard Value = 28183.8 nM. Clearly this is not a real biological value as it must relate to a systemic artifact (detection limit, placeholder, etc). Needs tracing to source assays before it could be included as a valid measurement.
...
### Comments
* Within the filtered dataset: 64666 (74%) inconclusive, 15848 (18%) inactive, 6478 (7%) active.
...
* **Decision:** excluding "inconclusive" is a scope choice (to measure resolved potency values only), rather than a data-quality correction. 'Inconclusive' rows will be filtered out and 'inactive' rows will be retained, as they represent a conclusive result.
    * New Standard Value mean = 9161.86, median = 10000.00. Standard Value skew = 1.36, excess kurtosis = 3.04.
...
* There are 0 rows below the lower fence, and 1330 above the upper fence (ca. 6%).
